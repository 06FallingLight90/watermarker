...
- **「应用当前设置到全部」按钮**：批处理面板新增按钮，一键将当前预览中的水印设置应用到队列中所有图片的独立配置

### 修复

- **批处理 EXIF 预览水印延后**：修复队列切换文件时，预览中的 EXIF 水印仍显示上一张图片元数据的问题。原因是 EXIF 数据加载晚于图片加载，且 `imageStore.exifData` 缺少响应式监听 — `useCanvas.ts` 新增 `exifData` watcher，`openBatchFile` 调整为 EXIF 先于图片加载，切换时也清除旧 EXIF
