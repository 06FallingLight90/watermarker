...
[Canvas] renderPreview()
    ...
    │ 4. renderWatermarkStatic(ctx, canvas, scale, store)
...
用户添加文件 → 选择输出目录 → 点击"开始批处理"
    │
    ▼
[BarchPanel] for each file:
    │ 1. [Rust] loadImageRaw(filePath) → 原始文件字节 → base64
    │ 2. [Canvas] renderOffscreen(rawBase64, format, store)
    │    └─ 与 renderFullRes 相同的离屏渲染逻辑
    │ 3. [Rust] export_file(renderedBase64, outPath)
    ▼
逐文件渲染进度反馈
```
...
系统字体列表由 Rust `list_system_fonts` 命令扫描平台字体目录获取，前端在 `onMounted` 时加载并填充下拉菜单。
...
- 将 `renderFullRes()` 和 `renderOffscreen()` 提取为**模块级函数**
...
App.vue
  ...
  │                                          │
  ├── RightPanel ────────────────────────────┘
  │   · 直接 import renderFullRes (模块级)
  │   · 修改 watermarkStore → 触发 Canvas 重绘
  │
  └── BatchPanel
